...
public final class WingsBaublesIntegration {
    ...
    @Mod.EventHandler
    public void initial(final FMLInitializationEvent event) {
        MinecraftForge.EVENT_BUS.register(new Object() {
            @SubscribeEvent
            public void onXPPickUp(final PlayerPickupXpEvent event) {
                EntityPlayer mp = event.getEntityPlayer();
                if (getEquippedWingsOrNull(mp) != ItemStack.EMPTY) {
                    ItemStack itemstack = getEquippedWingsOrNull(mp);
                    if (EnchantmentHelper.getEnchantmentLevel(Enchantments.MENDING, itemstack) > 0) {
                        if (!itemstack.isEmpty() && itemstack.isItemDamaged()) {
                            float ratio = itemstack.getItem().getXpRepairRatio(itemstack);
                            int i = Math.min(roundAverage(event.getOrb().xpValue * ratio), itemstack.getItemDamage());
                            event.getOrb().xpValue -= roundAverage(i / ratio);
                            itemstack.setItemDamage(itemstack.getItemDamage() - i);
                        }
                    }

                    if (event.getOrb().xpValue > 0) {
                        mp.addExperience(event.getOrb().xpValue);
                    }
            ...
            public ItemStack getEquippedWingsOrNull(EntityPlayer player) {
                IBaublesItemHandler handler = BaublesApi.getBaublesHandler(player);

                for (int a = 0; a < handler.getSlots(); ++a) {
                    if (!handler.getStackInSlot(a).isEmpty() && handler.getStackInSlot(a).getItem() instanceof ItemWings) {
                        return handler.getStackInSlot(a);
                    }
                }
